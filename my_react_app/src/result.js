  ...
  render() {
  	return (
  		<table className = "itemTable">
  			<thead>
  				<tr>
  					<th className = "itemHead" colSpan="2" onClick = {this.goDetail}>Number {this.props.sequence}: {this.props.title}</th>
  				</tr>
  			</thead>
  			<tbody>
  				<tr>
      ...
      <div className="Items">
        ...
        <div className = "main-page">
        	<div className = "itemList">
        		{this.state.items.map((node, index) => <Item key = {index} transcription = {node.transcription} date = {node.date} language = {node.language} findSpot = {node.findSpot} photo = {node.fotos} sequence = {node.sequence} title = {node.title} data = {node.data}/>)}
        	</div>
          <ReactPaginate
